...
def local_search_swap(vehicles):
    improved, curr_cost = False, calculate_total_tardiness(vehicles)
    for v in vehicles:
        for i in range(len(v.schedules)):
            for j in range(i+1, len(v.schedules)):
                ...
                if new_cost < curr_cost: curr_cost, improved=True; break
                else: v.schedules[i], v.schedules[j] = v.schedules[j], v.schedules[i]
            if improved: break
    return improved
...
def ils_optimize(instance: Instance, time_limit: float = 5.0) -> Solution:
    ...
    while time.time() - start < time_limit:
        ...
        if curr_cost < best_cost: best_cost, best_state = curr_cost, [list(v.schedules) for v in vehicles]
        # 약간의 섞기
        for v in vehicles:
...
def vns_optimize(instance: Instance, time_limit: float = 5.0) -> Solution:
    ...
    best_state = [list(v.schedules) for v in vehicles]
    start = time.time()
    neighborhoods = [local_search_swap, local_search_2opt]
    while time.time() - start < time_limit:
        for nb in neighborhoods:
            improved = nb(vehicles)
            if improved:
                curr_cost = calculate_total_tardiness(vehicles)
...
def sa_optimize(instance: Instance, time_limit: float = 5.0) -> Solution:
    ...
    while time.time() - start < time_limit and T > T_min:
        move_type = random.choice(['swap', '2opt'])
        if move_type == 'swap':
            v = random.choice([v for v in vehicles if len(v.schedules)>1])
            i, j = random.sample(range(len(v.schedules)),2)
            v.schedules[i], v.schedules[j] = v.schedules[j], v.schedules[i]
        else:
            v = random.choice([v for v in vehicles if len(v.schedules)>3])
            i = random.randint(0, len(v.schedules)-2)
            j = random.randint(i+2, len(v.schedules)-1)
            v.schedules[i:j+1] = reversed(v.schedules[i:j+1])
        new_cost = calculate_total_tardiness(vehicles)
        delta = new_cost - current_cost
        if delta < 0 or random.random() < math.exp(-delta / max(T, 1e-9)):
            current_cost = new_cost
            if new_cost < best_cost: best_cost, best_state = new_cost, [list(v.schedules) for v in vehicles]
        T *= alpha
    for v_idx, v in enumerate(vehicles): v.schedules = best_state[v_idx]
...
def ga_optimize(instance: Instance, time_limit: float = 5.0) -> Solution:
    ...
    start = time.time()
    while time.time() - start < time_limit:
        parents = [min(random.sample(population, min(5, len(population))), key=lambda x: x[0]) for _ in range(pop_size)]
        offspring = []
        for i in range(0, len(parents)-1, 2):
            p1_state, p2_state = parents[i][1], parents[i+1][1]
            child = copy.deepcopy(p1_state)
            if len(vehicles) > 0 and len(child[0]) > 2:
                v_idx = random.randint(0, len(child)-1)
                cut = len(child[v_idx]) // 2
                child[v_idx][:cut] = p2_state[v_idx][:cut]
...
def gurobi_optimize(instance: Instance, time_limit: float = 600) -> Optional[Solution]:
    ...
    for i in range(n):
        model.addConstr(quicksum(visit[v, i] for v in range(m)) == 1)
    for v in range(m):
        for i in range(n):
    ...
    for v in range(m):
        model.addConstr(quicksum(customers[i].weight * visit[v, i] for i in range(n)) <= vehicles[v].capacity)
    for v in range(m):
        for i in range(n):
            for j in range(n):
                if i != j:
                    model.addConstr(u[v, j] >= u[v, i] + 1 - n * (1 - x[v, i, j]))
    M = 100000
    for v in range(m):
        for i in range(n):
            for j in range(n):
