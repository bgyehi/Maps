...
def _make_output_dir(output_dir="output"):
    """Create output directory for HTML maps."""
    import os
...
def scenario_easy():
    """
    Easy scenario:
    - 4 EV charging customers
    - 3 delivery satellites
    - 2 vehicles
    - short time limit
    """
    print("\n" + "=" * 80)
    ...
    print("=" * 80)
    print("Setting: 4 customers, 3 satellites, 2 vehicles, time limit 30 sec")
    print("-" * 80)

    coords = [
        (37.5665, 126.9780),  # Depot: Seoul City Hall
        (37.5729, 126.9769),  # C1: Gwanghwamun
        (37.5663, 126.9900),  # C2: Euljiro/Myeongdong area
        (37.5547, 126.9706),  # C3: Seoul Station
        (37.5700, 126.9920),  # C4: Jongno area
    ]
    ...
    sat_coords = [
        ...
        (37.5655, 126.9925),  # S1 near C2/C4
    ...
    if result:
        print_scenario_result(result, scenario_label="Easy")

        output_dir = _make_output_dir("output")
        output_html = f"{output_dir}/scenario_easy_map.html"
        draw_solution_map(
            coords=coords,
            sat_coords=sat_coords,
            result=result,
...
def scenario_hard():
    """
    Hard scenario with ambiguous delivery ownership:
    - 12 EV charging customers
    - 14 delivery satellites
    - several satellites are intentionally placed between two or more
      charging-customer hubs, so the model must decide whether vehicle A
      or vehicle B should handle the nearby delivery while visiting its
      charging customer.
    - This creates ambiguity through z[s, i, k]: satellite s can be assigned
      to one of several feasible hubs i and therefore to the vehicle k that
      visits that hub.
    """
    print("\n" + "=" * 80)
    print("SCENARIO 2: HARD PROBLEM - AMBIGUOUS SATELLITE ASSIGNMENT")
    print("=" * 80)
    print("Setting: 12 customers, 14 satellites, 4 vehicles, time limit 300 sec")
    print("Key idea: boundary satellites can be served by multiple nearby charging hubs")
    print("-" * 80)

    # Charging customer locations.
    # C9, C10, C11, C12 are added/positioned so that some delivery satellites
    # sit between two likely charging stops. This makes the assignment ambiguous:
    # the model decides which charging stop becomes the hub and which vehicle
    # performs the small satellite delivery tour.
    coords = [
        (37.5665, 126.9780),  # Depot: Seoul City Hall
        (37.5729, 126.9769),  # C1: Gwanghwamun
        ...
        (37.5700, 126.9920),  # C9: Jongno/Euljiro boundary
        (37.5512, 126.9882),  # C10: Namsan/Itaewon edge
        (37.5860, 127.0015),  # C11: Hyehwa/Daehakro
        (37.5400, 126.9920),  # C12: Yongsan/Itaewon
    ]
    ...
    # Delivery satellites.
    # Several are deliberately placed in boundary zones:
    # - S0, S1: between C1 and C9
    # - S2, S3: between C3 and C10/C12
    # - S7, S10: between C5 and C8 or C2/C8
    # - S8, S11: between C9 and C11
    # - S9, S12: between C10 and C12
    # With R_km=6.0, these satellites normally have multiple feasible hubs.
    sat_coords = [
        (37.5715, 126.9845),  # S0 boundary C1-C9
        (37.5688, 126.9825),  # S1 boundary C1-C9-C3
        (37.5538, 126.9805),  # S2 boundary C3-C10
        (37.5465, 126.9825),  # S3 boundary C10-C12
        (37.5195, 126.9270),  # S4 near C4
        (37.4995, 127.0310),  # S5 near C5
        (37.5115, 127.0965),  # S6 near C6
        (37.5215, 127.0450),  # S7 boundary C5-C8
        (37.5795, 126.9970),  # S8 boundary C9-C11
        (37.5455, 126.9935),  # S9 boundary C10-C12
        (37.5555, 127.0340),  # S10 boundary C2-C8
        (37.5758, 126.9895),  # S11 boundary C1-C9-C11
        (37.5368, 126.9888),  # S12 boundary C10-C12
        (37.5575, 126.9215),  # S13 near C7
    ]
    ...
    if result:
        print_scenario_result(result, scenario_label="Hard - Ambiguous Assignment")

        output_dir = _make_output_dir("output")
        output_html = f"{output_dir}/scenario_hard_ambiguous_map.html"
        draw_solution_map(
            coords=coords,
            sat_coords=sat_coords,
            result=result,
...
def print_scenario_result(result, scenario_label="Scenario"):
    """Print a compact result summary for scenario experiments."""
    print(f"\n=== {scenario_label} Result ===")
    ...
    ambiguous_satellites = []
    for s, hubs in result["feasible_hubs_by_satellite"].items():
        hub_text = ", ".join(f"C{i}" for i in hubs)
        marker = "  <-- ambiguous" if len(hubs) >= 2 else ""
        print(f"  S{s}: " + hub_text + marker)
        if len(hubs) >= 2:
            ambiguous_satellites.append(s)

    if ambiguous_satellites:
        print(
            "\nAmbiguous delivery satellites "
            "(candidate hubs >= 2): "
            + ", ".join(f"S{s}" for s in ambiguous_satellites)
        )
...
def compare_scenarios(res_easy, res_hard):
    """Print comparison table for Easy and Hard scenarios."""
    print("\n" + "=" * 80)
    print("SCENARIO COMPARISON")
    print("=" * 80)

    if not res_easy or not res_hard:
        print("Comparison skipped because at least one scenario has no solution.")
        return

    easy_total_dist = (
        res_easy["vehicle_distance_km"]
        + res_easy["satellite_tour_distance_km"]
    )
    hard_total_dist = (
        res_hard["vehicle_distance_km"]
        + res_hard["satellite_tour_distance_km"]
    )

    print(f"\n{'Metric':<28} {'Easy':>18} {'Hard':>18}")
    print("-" * 68)
    print(f"{'Customers':<28} {4:>18} {12:>18}")
    print(f"{'Satellites':<28} {3:>18} {14:>18}")
    print(f"{'Vehicles used':<28} {res_easy['num_vehicles_used']:>18} {res_hard['num_vehicles_used']:>18}")
    print(f"{'Objective':<28} {res_easy['objective']:>18.2f} {res_hard['objective']:>18.2f}")
    print(f"{'Vehicle distance km':<28} {res_easy['vehicle_distance_km']:>18.2f} {res_hard['vehicle_distance_km']:>18.2f}")
    print(f"{'Satellite distance km':<28} {res_easy['satellite_tour_distance_km']:>18.2f} {res_hard['satellite_tour_distance_km']:>18.2f}")
    print(f"{'Total distance km':<28} {easy_total_dist:>18.2f} {hard_total_dist:>18.2f}")
    print(f"{'Satellite duration min':<28} {res_easy['satellite_tour_duration_min']:>18.2f} {res_hard['satellite_tour_duration_min']:>18.2f}")


if __name__ == "__main__":
    print("\n" + "=" * 80)
    print("CVRPTW + Satellite Tours Scenario Experiment")
    print("=" * 80)

    res_easy = scenario_easy()
    res_hard = scenario_hard()

    compare_scenarios(res_easy, res_hard)
