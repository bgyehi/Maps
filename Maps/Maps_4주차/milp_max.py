...
from module_ import Instance, Schedule, get_obj


def milp_scheduling(_prob: Instance, time_limit=300):
    ...
    max_s = int(np.max(np.array(s))) if s else 0
    ...
    z_ijk = {(i, j, k): model.binary_var(name=f"z_{i}_{j}_{k}") for i in SJ for j in SJ for k in SM if i < j}
    ...
    for k in SM:
        ...
        if hasattr(machine, "assigned"):
            machine.assigned = []
        if hasattr(machine, "available"):
            machine.available = 0
        if hasattr(machine, "ptime"):
            machine.ptime = prob.ptime[k]
        if hasattr(machine, "setup"):
            machine.setup = prob.setup[k]
    ...
    # 추가
    try:
        sched.gap = model.solve_details.mip_relative_gap
    except Exception:
        sched.gap = None
